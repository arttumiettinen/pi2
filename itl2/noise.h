...
{
	/**
	Adds noise to pixels of the image. Noise values are drawn from the generator given as argument.
	...
	*/
	...
	@param mean Mean of the normal distribution where noise samples are drawn.
	@param stddev Standard deviation of the distribution where the noise samples are drawn.
	*/
	template<typename pixel_t> void noise(Image<pixel_t>& img, double mean = 0, double stddev = 25)
	{
		std::mt19937 gen;
