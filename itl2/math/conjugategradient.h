...
namespace itl2
{
	...
	*/
	template<typename real_t = double> void conjugateGradient(const Matrix<real_t>& A, const Matrix<real_t>& b, Matrix<real_t>& x, const real_t tolerance = NumberUtils<real_t>::tolerance(), const size_t maxiter = std::numeric_limits<size_t>::max())
	{
		if (!A.isSymmetric())
		{
	...
	/**
	Solves A^T A x = A^T b using conjugate gradient method.
	*/
	template<typename real_t = double> void cgne(const Matrix<real_t>& A, const Matrix<real_t>& b, Matrix<real_t>& x, const real_t tolerance = NumberUtils<real_t>::tolerance(), const size_t maxiter = std::numeric_limits<size_t>::max())
	{
	...
	{
		/**
		Test for conjugate gradient algorithm.
		*/
		...
		/**
		Test for conjugate gradient on normal equations.
		*/
		inline void cgne()
		{
			Matrix<double> A(3, 2);
